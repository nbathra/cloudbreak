...
public interface InstanceMetaDataRepository extends DisabledBaseRepository<InstanceMetaData, Long> {
    ...
    @Query("SELECT i FROM InstanceMetaData i WHERE i.stackId= :stackId AND i.instanceStatus <> 'TERMINATED'")
    Set<InstanceMetaData> findNotTerminatedForStack(@Param("stackId") Long stackId);

    @Query("SELECT i FROM InstanceMetaData i WHERE i.stackId= :stackId")
    Set<InstanceMetaData> findAllInStack(@Param("stackId") Long stackId);

    @Query("SELECT i FROM InstanceMetaData i WHERE i.instanceId= :instanceId AND i.stackId= :stackId")
    InstanceMetaData findByInstanceId(@Param("stackId") Long stackId, @Param("instanceId") String instanceId);

    @Query("SELECT i FROM InstanceMetaData i WHERE i.stackId= :stackId AND i.discoveryFQDN= :hostName AND i.instanceStatus <> 'TERMINATED'")
    ...
    @Query("SELECT i FROM InstanceMetaData i WHERE i.stackId= :stackId AND i.privateIp= :privateAddress AND i.instanceStatus <> 'TERMINATED'")
    InstanceMetaData findNotTerminatedByPrivateAddress(@Param("stackId") Long stackId, @Param("privateAddress") String privateAddress);

    @Query("SELECT i.serverCert FROM InstanceMetaData i WHERE i.stackId= :stackId AND i.instanceMetadataType = 'GATEWAY_PRIMARY' "
